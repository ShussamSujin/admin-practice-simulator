...
const sections: Section[] = [
  { id:'labs', title:'연수 실습실', subtitle:'연수 자료의 핵심 과제를 순서대로 실습', icon:BookOpenCheck, links:['전체 실습','OU와 그룹','우회 차단','앱과 확장','화이트리스트','기기 등록','Gemini','계정과 진급','보안과 인수인계'] },
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (!signedIn) return;
    const key=`admin-c-practice-v2:${profile.email || 'guest'}`;
  ...
  useEffect(() => {
    if (!signedIn) return;
    localStorage.setItem(`admin-c-practice-v2:${profile.email || 'guest'}`,JSON.stringify({orgs,users,groups}));
...
function SectionView({section,activeLink,setActiveLink,users,orgs,groups,onModal,onDeleteOrg,onDeleteUser,onDeleteGroup,onAction,onHome}:{section:Section;activeLink:string;setActiveLink:(v:string)=>void;users:PracticeUser[];orgs:Org[];groups:PracticeGroup[];onModal:(v:'org'|'user'|'group')=>void;onDeleteOrg:(id:string)=>void;onDeleteUser:(id:string)=>void;onDeleteGroup:(id:string)=>void;onAction:(v:string)=>void;onHome:()=>void}){
  ...
  if(section.id==='labs') return <TrainingLabs activeLink={activeLink} setActiveLink={setActiveLink} onAction={onAction} onHome={onHome}/>;
...
const labModules=[
  {key:'OU와 그룹',title:'OU 설계 · 상속 · 그룹 우선순위',path:'디렉터리 > 조직 단위 / 그룹',description:'정책은 위에서 아래로 상속되고, 그룹 정책은 OU보다 우선한다는 흐름을 연습합니다.',controls:[['학교급','중등','초등|중등'],['최상위 기본 정책','안전하게 차단','안전하게 차단|기본 허용'],['학생 OU 정책','상속됨','상속됨|재정의됨'],['예외 처리','그룹 우선','그룹 우선|OU 우선']]},
  {key:'우회 차단',title:'학생 OU 4대 우회 차단',path:'기기 > Chrome > 설정',description:'시크릿·개인 계정·개발자 도구·VPN 확장을 모두 막는 조합을 구성합니다.',controls:[['시크릿 모드','사용 중지','사용 가능|사용 중지|강제로 사용'],['로그인 제한','*@school.sen.ms.kr','모든 사용자 허용|*@school.sen.ms.kr|로그인 허용 안함'],['게스트 모드','사용 안 함','사용|사용 안 함'],['개발자 도구','사용 중지','허용|사용 중지'],['VPN·프록시 확장','차단, Chrome에서 제거','설치 허용|차단|차단, Chrome에서 제거']]},
  {key:'앱과 확장',title:'앱 · 확장 프로그램 배포',path:'기기 > Chrome > 앱 및 확장 프로그램',description:'대상 OU 또는 그룹을 고르고 설치 정책과 고정 방식을 정합니다.',controls:[['배포 대상','학생 OU','교직원 OU|학생 OU|시험용 OU|선택 그룹'],['앱 유형','학습관리','학습관리|화면 녹화|카메라·녹음|VPN·프록시|게임·스트리밍'],['설치 정책','강제 설치 및 고정','강제 설치|강제 설치 및 고정|설치 허용|차단|차단, Chrome에서 제거'],['사용자 비활성화','허용 안 함','허용|허용 안 함']]},
  {key:'화이트리스트',title:'URL 차단 · 허용 목록',path:'사용자 및 브라우저 설정 > URL 차단',description:'차단 목록의 *에서 출발해 인증·Wi-Fi·업데이트·학습 사이트를 범주별로 허용합니다.',controls:[['운영 방식','화이트리스트','블랙리스트|화이트리스트'],['차단 목록','*','*|개별 URL 차단'],['허용 범주','인증 인프라','인증 인프라|학습 플랫폼|교과서·출판사|학교·교육청|검색·백과|에듀테크|보이지 않는 인프라'],['허용 패턴','classroom.google.com','google.com|classroom.google.com|go.kr|https://site.kr/study']]},
  {key:'기기 등록',title:'기기 등록 · OU 배치 · 검증',path:'기기 > ChromeOS 기기',description:'CEU 라이선스, 강제 재등록, OU 배치와 chrome://policy 검증을 순서대로 실습합니다.',controls:[['라이선스','번들','번들|단품'],['등록 상태','기업 등록 완료','미등록|기업 등록 완료|등록 해제'],['강제 재등록','기본값 사용','기본값 사용|사용자 인증정보로 강제|강제하지 않음'],['기기 OU','시험용','교직원|학생|공용기기|시험용'],['정책 검증','확인','대기|오류|확인']]},
  {key:'Gemini',title:'Gemini 3단계 허용 설계',path:'앱 > 추가 Google 서비스 > Gemini',description:'도메인은 닫고, 교직원 OU와 파일럿 그룹만 여는 단계별 정책을 구성합니다.',controls:[['연령 기반 접근','18세 미만 포함','18세 미만 포함|전체 18세 이상'],['도메인 전체','OFF','ON|OFF'],['교직원 OU','ON','ON|OFF|상속'],['학생 OU','OFF','ON|OFF|상속'],['파일럿 그룹','ON','ON|OFF|미설정'],['시험용 OU','OFF 고정','ON|OFF 고정']]},
  {key:'계정과 진급',title:'계정 일괄 생성 · OU 경로 · 진급',path:'디렉터리 > 사용자 > 일괄 업데이트',description:'CSV 열과 OU 경로를 확인하고 위 학년부터 이동하는 진급 순서를 연습합니다.',controls:[['계정 규칙','입학연도+순번','학번 기반|이름+숫자|입학연도+순번'],['CSV 필수 열','이름·성·이메일·비밀번호·OU','이름·성·이메일·비밀번호·OU|이메일만'],['OU 경로','/학생/1학년','학생/1학년|/학생/1학년|/학생/1학년/'],['진급 첫 단계','3학년 → 졸업보관','1학년 → 2학년|3학년 → 졸업보관'],['졸업생 계정','졸업보관으로 이동','즉시 삭제|졸업보관으로 이동']]},
  {key:'보안과 인수인계',title:'관리자 보안 · 역할 · 인수인계',path:'보안 / 관리자 역할 / 보고',description:'2단계 인증, 최소 권한, 변경 기록과 인수인계 항목을 점검합니다.',controls:[['최고 관리자 수','2명 이상','1명|2명 이상'],['2단계 인증','강제','선택|강제'],['학생 관리 권한','최소 범위 위임','최고 관리자 부여|최소 범위 위임'],['변경 기록','변경 전·후·검증 방법','날짜만|변경 전·후·검증 방법'],['인수인계 위치','공유 드라이브','개인 드라이브|공유 드라이브']]},
];

function TrainingLabs({activeLink,setActiveLink,onAction,onHome}:{activeLink:string;setActiveLink:(v:string)=>void;onAction:(v:string)=>void;onHome:()=>void}){const [values,setValues]=useState<Record<string,string>>({});const [done,setDone]=useState<string[]>([]);useEffect(()=>{try{const data=JSON.parse(localStorage.getItem('admin-c-labs')||'{}');setValues(data.values||{});setDone(data.done||[])}catch{}},[]);const save=(key:string,value:string)=>{const next={...values,[key]:value};setValues(next);localStorage.setItem('admin-c-labs',JSON.stringify({values:next,done}))};const complete=(key:string)=>{const next=done.includes(key)?done.filter(x=>x!==key):[...done,key];setDone(next);localStorage.setItem('admin-c-labs',JSON.stringify({values,done:next}));onAction(`${key} 실습 ${next.includes(key)?'완료':'다시 시작'}`)};const visible=activeLink==='전체 실습'||activeLink==='개요'?labModules:labModules.filter(l=>l.key===activeLink);return <div className="section-page wide labs-page"><nav className="breadcrumbs"><button onClick={onHome}>홈</button><ChevronRight size={14}/><strong>연수 실습실</strong></nav><div className="labs-heading"><div><p className="eyebrow">2026 디벗 테크니션 연수 연계</p><h1>관리콘솔 마법서 · 실습실</h1><p>공유해주신 연수 자료의 핵심 설정을 실제 화면 흐름처럼 연습합니다.</p></div><div className="progress-ring"><strong>{done.length}/{labModules.length}</strong><span>완료</span></div></div><div className="lab-tabs">{['전체 실습',...labModules.map(l=>l.key)].map(name=><button className={activeLink===name||(activeLink==='개요'&&name==='전체 실습')?'active':''} key={name} onClick={()=>setActiveLink(name)}>{name}</button>)}</div><div className="labs-grid">{visible.map((lab,index)=><article className={`lab-card ${done.includes(lab.key)?'done':''}`} key={lab.key}><header><span>LAB {index+1}</span><b>{done.includes(lab.key)?'완료됨':'실습 중'}</b></header><h2>{lab.title}</h2><p>{lab.description}</p><small>{lab.path}</small><div className="lab-controls">{lab.controls.map(([label,initial,options])=>{const id=`${lab.key}:${label}`;return <label key={id}><span>{label}</span><select value={values[id]||initial} onChange={e=>save(id,e.target.value)}>{options.split('|').map(option=><option key={option}>{option}</option>)}</select></label>})}</div><div className="lab-note"><label><span>연수 메모</span><textarea value={values[`${lab.key}:메모`]||''} onChange={e=>save(`${lab.key}:메모`,e.target.value)} placeholder="이 설정을 선택한 이유나 확인 결과를 적어 보세요."/></label></div><button className="complete-button" onClick={()=>complete(lab.key)}>{done.includes(lab.key)?'완료 취소':'이 실습 완료'}</button></article>)}</div></div>}
...
function SettingsView({section,activeLink,setActiveLink,onAction,onHome}:{section:Section;activeLink:string;setActiveLink:(v:string)=>void;onAction:(v:string)=>void;onHome:()=>void}){
  const Icon=section.icon; const [tab,setTab]=useState('사용자 및 브라우저 설정'); const [editing,setEditing]=useState<string|null>(null); const [saved,setSaved]=useState<Record<string,string>>({});
  const settingsBySection:Record<string,string[]>={
    chrome:['사용자 세션 최대 길이','Chrome 브라우저 로그인 제한','게스트 모드 사용','시크릿 모드 사용','브라우저 기록 저장','홈페이지 URL','시작 시 열리는 페이지','URL 차단 목록','URL 허용 목록','세이프 브라우징','다운로드 제한','비밀번호 관리자','브라우저 동기화','관리 북마크','확장 프로그램 설치 정책','외부 확장 프로그램 차단','인쇄 허용','스크린샷 허용','맞춤 배경화면','맞춤 테마 색상','브라우저 업데이트 채널','브라우저 다시 시작 알림'],
    devices:['기기 자동 등록','로그인 화면 사용자 이름 표시','게스트 탐색 허용','로그인 제한','키오스크 앱','자동 업데이트','업데이트 채널','업데이트 고정 버전','기기 보고','하드웨어 정보 보고','유휴 상태 전원 관리','예약된 재부팅','USB 기기 허용','외부 저장장치','Wi-Fi 네트워크','인증서 관리','프린터 설정','원격 데스크톱','분실 기기 사용 중지'],
    security:['2단계 인증 시행','보안 키 허용','비밀번호 길이','비밀번호 재사용 방지','로그인 시도 차단','세션 제어','계정 복구','SSO 프로필','컨텍스트 인식 액세스','API 제어','앱 액세스 제어','경고 규칙'],
    apps:['Gmail 서비스 상태','Drive 외부 공유','Calendar 외부 공유','Meet 녹화 허용','Chat 외부 사용자','Classroom 수업 만들기','Marketplace 앱 설치','추가 Google 서비스'],
  };
  const settings=settingsBySection[section.id]||['서비스 상태','사용자 액세스','외부 공유','데이터 보관','알림 설정','감사 로그','기본 정책','조직 단위 재정의','관리자 권한','사용 현황 보고'];
  useEffect(()=>{try{const value=localStorage.getItem(`admin-c-settings:${section.id}`);if(value)setSaved(JSON.parse(value))}catch{}},[section.id]);
  const savePolicy=(name:string,value:string)=>{const next={...saved,[name]:value};setSaved(next);localStorage.setItem(`admin-c-settings:${section.id}`,JSON.stringify(next));setEditing(null);onAction(`${name} 연습 설정 저장`)};
  return <div className="section-page wide"><nav className="breadcrumbs"><button onClick={onHome}>홈</button><ChevronRight size={14}/><span>{section.title}</span><ChevronRight size={14}/><strong>{activeLink}</strong></nav><div className="settings-shell"><aside className="settings-nav"><h1>설정</h1>{section.links.map(link=><button className={activeLink===link?'active':''} key={link} onClick={()=>setActiveLink(link)}>{link}<ChevronDown size={17}/></button>)}</aside><section className="settings-main"><div className="settings-tabs">{['사용자 및 브라우저 설정','기기 설정','관리 게스트 세션 설정'].map(name=><button key={name} className={tab===name?'active':''} onClick={()=>setTab(name)}>{name}</button>)}</div><div className="settings-tools"><span>표시: 지원됨</span><button onClick={()=>onAction('설정 필터 추가')}><Plus size={18}/> 필터 검색 또는 추가</button><button onClick={()=>onAction('최근 변경사항 보기')}>최근 변경사항</button></div><div className="settings-title"><span className="section-icon"><Icon size={22}/></span><div><strong>{activeLink} · {tab}</strong><small>{settings.length}개의 연습 정책 · 항목을 눌러 직접 변경</small></div></div><div className="settings-table"><div className="settings-row header"><b>설정</b><b>구성</b><b>상속</b><b>지원 플랫폼</b></div>{settings.map((name,i)=><button className="settings-row" key={name} onClick={()=>setEditing(name)}><span>{name}</span><span className={saved[name]?'saved-value':''}>{saved[name]||'Google 기본값'}</span><span>{saved[name]?'로컬 단위로 적용됨':'상속됨'}</span><span className="platforms">▣ ◉ ▲ iOS</span></button>)}</div></section></div>{editing&&<PolicyEditor name={editing} section={section.title} current={saved[editing]} onClose={()=>setEditing(null)} onSave={value=>savePolicy(editing,value)}/>}</div>
}

function PolicyEditor({name,section,current,onClose,onSave}:{name:string;section:string;current?:string;onClose:()=>void;onSave:(v:string)=>void}){const [mode,setMode]=useState(current||'허용');const [scope,setScope]=useState('선택한 조직 단위');const [detail,setDetail]=useState('');return <div className="modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}><section className="modal policy-editor"><header><div><p className="eyebrow">{section} · 연습 정책 편집</p><h2>{name}</h2><p>실제 Chrome 브라우저나 기기에는 적용되지 않습니다.</p></div><button className="icon-button" onClick={onClose}><X/></button></header><div className="policy-body"><fieldset><legend>정책 값</legend>{['허용','차단','사용자 선택 허용','Google 기본값 사용'].map(value=><label key={value}><input type="radio" name="policy-value" checked={mode===value} onChange={()=>setMode(value)}/><span>{value}</span></label>)}</fieldset><label><span>적용 범위</span><select value={scope} onChange={e=>setScope(e.target.value)}><option>선택한 조직 단위</option><option>최상위 조직 단위</option><option>교직원</option><option>학생</option><option>1학년</option></select></label><label><span>세부 값 / URL / 시간 (선택)</span><textarea value={detail} onChange={e=>setDetail(e.target.value)} placeholder="예: https://school.example 또는 60분"/></label><label className="switch-line"><input type="checkbox"/><span><strong>하위 조직 단위에 강제 적용</strong><small>연습 화면의 상속 표시가 재정의로 바뀝니다.</small></span></label><div className="policy-preview"><strong>연습 적용 미리보기</strong><span>{scope} · {mode}{detail?` · ${detail}`:''}</span></div><div className="form-actions"><button onClick={onClose}>취소</button><button onClick={()=>onSave(detail?`${mode} · ${detail}`:mode)}>연습용으로 저장</button></div></div><div className="modal-warning"><ShieldCheck size={16}/> 이 정책은 브라우저 안의 가짜 연습 데이터입니다.</div></section></div>}
